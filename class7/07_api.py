...
# what happens if the movie name is not recognized? ERROR HANDLING
r = requests.get('http://www.omdbapi.com/?t=blahblah&r=json&type=movie')
r.status_code
r.text #returns error movie not found. 

# define a function to return the year
def get_movie_year(title):
    r = requests.get('http://www.omdbapi.com/?t=' + title + '&r=json&type=movie')
  #  r = requests.get('http://www.omdbapi.com/?t=' + str(title) + '&r=json&type=movie')  #to be more careful
    info = r.json() # make it into dictionary
...
for title in top_movies.title:
    years.apoend(get_movie_year(title))
    sleep(1) # puts a break of 1 second between gets to be kind to be API
...
letters = ['a', 'b', 'c']
for index, letter in enumerate(letters):  #keeps track of index in a loop
    print (index, letter)
