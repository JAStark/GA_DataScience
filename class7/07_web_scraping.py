...
# Tags allow you to access the 'inside text' returns as a str
b.find(name='h1').text
...
b.find_all(name='p')[0]['id']
b.find(name='p')[0]['id']  #both work

# print the text of all four resources
b.find_all(name='ul').find_all(name='li').text  #WRONG!
results = b.find_all(name='li')
for tag in results:
    print(tag.text)

# print the text of only the API resources
b.find(name='ul', attrs={'id':'api'}).find_all(name='li').text  #WRONG!
...
import sys
reload(sys)
sys.setdefaultsencoding('utf8')

# get the title
b.find(name='span', attrs={'class':'itemprop', 'itemprop':'name'}).text
'''FUTURE good to do find_all first to check if there more of these things.'''
...
# get the description
b.find(name='p', attrs={'itemprop':'description'}).text.strip() #gets rid of \n character
